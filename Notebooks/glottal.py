...
class Glottal:
    ...
    def read_audio_file(self, file_path):
        """
        Reads an audio file and returns the sampling rate and audio data.
        Supports both .wav and .mp3 files.

        :param file_path: Path to the audio file.
        :returns: sampling rate, audio data
        """
        data_audio, fs = librosa.load(file_path, sr=None, mono=True)
        print("used general")
        return fs, data_audio

    def extract_features_file(self, audio, static=True, plots=False, fmt="npy", kaldi_file="", general=False):
        ...
        if not general and audio.find('.wav') == -1 and audio.find('.WAV') == -1:
            raise ValueError(audio+" is not a valid wav file")
        
        fs, data_audio = read(audio) if not general else self.read_audio_file(audio)
